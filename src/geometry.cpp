...
void Geometry::line(float _x0, float _y0, float _z0, float _x1, float _y1,
                    float _z1, float *_zbuffer, const TGAColor &_color) const {
    // 斜率大于 1
    bool steep = false;
    ...
    for (float x = _x0; x <= _x1; x++) {
        idx = (size_t)(x + y * image.get_width());
        if (steep == true) {
            if (_zbuffer[idx] < z) {
                image.set(y, image.get_height() - x, _color);
                _zbuffer[idx] = z;
...
void Geometry::triangle(const Vectors2 &_vertex1, const Vectors2 &_vertex2,
                        const Vectors2 &_vertex3,
                        const TGAColor &_color) const {
    line(_vertex1, _vertex2, _color);
    line(_vertex2, _vertex3, _color);
    line(_vertex3, _vertex1, _color);
    return;
}
...
void Geometry::triangle(const Vectorf3 &_vertex1, const Vectorf3 &_vertex2,
                        ...
                        const TGAColor &_color) const {
    line(_vertex1, _vertex2, _zbuffer, _color);
    line(_vertex2, _vertex3, _zbuffer, _color);
    line(_vertex3, _vertex1, _zbuffer, _color);
    return;
}
...
void Geometry::triangle_fill(const Vectors2 &_vertex1, const Vectors2 &_vertex2,
                             const Vectors2 &_vertex3,
                             const TGAColor &_color) const {
    // 遍历平行四边形，如果在三角形内则填充
...
void Geometry::triangle_fill(const Vectors2 *_vertexes,
                             const TGAColor &_color) const {
    triangle_fill(_vertexes[0], _vertexes[1], _vertexes[2], _color);
    return;
}

void Geometry::triangle_fill(const Vectorf3 &_vertex1, const Vectorf3 &_vertex2,
                             const Vectorf3 &_vertex3, float *_zbuffer,
                             const TGAColor &_color) const {
    ...
    for (p.coord.x = min.coord.x; p.coord.x <= max.coord.x; p.coord.x++) {
        for (p.coord.y = min.coord.y; p.coord.y <= max.coord.y; p.coord.y++) {
            if (is_barycentric(_vertex1, _vertex2, _vertex3, p) == true) {
                idx = (size_t)(p.coord.x + p.coord.y * image.get_width());
...
void Geometry::triangle_fill(const Vectorf3 *_vertexes, float *_zbuffer,
                             const TGAColor &_color) const {
    triangle(_vertexes[0], _vertexes[1], _vertexes[2], _zbuffer, _color);
    return;
}
...
void Geometry::circle(size_t _x0, size_t _y0, float _r, TGAColor _color) const {
    ...
    while (y > x) {
        ...
        x2 = -x + _x0;
        x3 = -y + _x0;
        y0 = image.get_height() - (y + _y0);
        y1 = image.get_height() - (x + _y0);
        y2 = image.get_height() - (-y + _y0);
        y3 = image.get_height() - (-x + _y0);
        image.set(x0, y0, _color);
        image.set(x1, y1, _color);
        image.set(x2, y0, _color);
        image.set(x3, y1, _color);
        image.set(x2, y2, _color);
        image.set(x3, y3, _color);
        image.set(x0, y2, _color);
        image.set(x1, y3, _color);
        if (d < 0) {
            d = d + 2 * x + 3;
        }
        else {
...
// TODO
void Geometry::circle_fill(size_t _x0, size_t _y0, float _r,
...
// TODO
void Geometry::circle_fill(size_t _x0, size_t _y0, float _r, float *_zbuffer,
                           TGAColor _color) const {
    assert(_r > 0);
    size_t x = 0;
    size_t y = _r;
